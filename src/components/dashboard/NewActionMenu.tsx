...
const NewActionMenu: React.FC<NewActionMenuProps> = ({ isOpen, onClose, sidebarOpen, buttonRef }) => {
  ...
  const getMenuPosition = () => {
    if (!buttonRef.current) return { top: '80px', left: '16px' }; // Moved down by 10px
    
    const buttonRect = buttonRef.current.getBoundingClientRect();
    return {
      top: `${buttonRect.bottom + window.scrollY + 10}px`, // Added 10px offset
  ...
  const categories = [
    {
      title: "Customers",
      items: [
        { title: "Invoice", to: "/dashboard/sales/invoice" },
